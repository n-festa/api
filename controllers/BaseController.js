	...
	static async getById(req, modelName) {
		...
		try{
			result = await req.app.get('db')[modelName].findByPk(reqParam).then(
				...
				errHandler.throwError(500, 'sequelize error, something wrong with either the data base connection or schema')
			)
		}catch(err){
	...
	static async getList(req, modelName, options){
		...
		try {
			...
			results = await req.app.get('db')[modelName]
				.findAll(options)
				.then(
					errHandler.throwIf(r => !r, 500 , 'Internal serve error' , 'something went wrong while fetching data'),
					errHandler.throwError(500, 'sequelize error')					
